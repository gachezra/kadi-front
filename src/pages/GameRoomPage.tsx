...
export const GameRoomPage: React.FC = () => {
  ...
  const isMyTurn =
    ...
    playerList[room.currentPlayer]?.userId === user?.id;
  const canStart =
    isOwner && room?.status === "waiting" && playerList.length >= 2;

  useEffect(() => {
    if (
      room &&
      user &&
      !playerList.some((p) => p.userId === user.id) &&
      !room.isTerminated
    ) {
  ...
  const handleTerminateRoom = async () => {
    if (
      !room ||
      !user ||
      !window.confirm("Are you sure you want to terminate this room?")
    )
      return;
    setIsActionLoading(true);
    try {
  ...
  const performMove = async (action: "drop" | "pick") => {
    ...
    try {
      ...
      if (response?.awaitingSpecialAction) {
        ...
        else if (specialCardValue && ["8", "Q"].includes(specialCardValue))
  ...
  const handleDeclareNikoKadi = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to declare Niko Kadi"
      );
    } finally {
      setIsActionLoading(false);
    }
      ...
      <GlassCard className="p-6">
        ...
        <div className="space-y-4 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="text-yellow-400 capitalize">{room.status}</span>
          </div>
        </div>
        {canStart && (
          <div className="mt-6 p-4 bg-green-900/50 rounded-lg border border-green-400/30 text-center">
            <p className="text-green-300">Ready to start!</p>
          </div>
        )}
      </GlassCard>
    </div>
  ...
  const GameView = () => {
    const renderTopCard = () => {
      ...
      if (isSuit) {
        ...
        const color = ["hearts", "diamonds"].includes(room.topCard)
          ? "text-red-500"
          : "text-white";
        return (
            ...
            <GlassCard className="p-6">
              <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-2">
                <h3 className="text-lg font-bold text-white">
                  Your Hand ({myPlayer.hand.length})
                </h3>
                ...
                {myPlayer.hand.map((card) => (
                  <PlayingCard
                    ...
                    isSelected={selectedCards.some(
                      (c) => cardToString(c) === cardToString(card)
                    )}
    ...
    <GlassCard className="p-8 text-center max-w-md mx-auto">
      ...
      {room.winner ? (
        <p className="text-xl text-gray-300">
          {playerList.find((p) => p.userId === room.winner)?.username ||
            "Unknown"}{" "}
          wins!
        </p>
      ) : (
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <Modal
        ...
        onClose={() => setShowSuitSelector(false)}
        title="Choose Suit"
      >
        {Object.entries(CARD_SUITS).map(([suit, symbol]) => (
          <Button
            key={suit}
            variant="primary"
          ...
          >
            {symbol} {suit.charAt(0).toUpperCase() + suit.slice(1)}
          </Button>
        ))}
      </Modal>
      <Modal
      ...
      >
        <div className="space-y-4">
          <p className="text-gray-300">
            You played a question card! Ask a question:
          </p>
          <input
            type="text"
          ...
          <div className="flex justify-end gap-3">
            ...
            >
              {isActionLoading ? <LoadingSpinner size="sm" /> : "Submit"}
            </Button>
          </div>
        </div>
      </Modal>
